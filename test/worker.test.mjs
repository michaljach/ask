...
// Colour is off for a non-terminal agent, which is also the readable shape to assert on.
r = await worker.fetch(new Request("https://ask.dev/", { headers: { "user-agent": "Mozilla/5.0" } }), ENV);
const plain = await r.text();
check("no colour for a browser agent", !plain.includes("\u001b["), JSON.stringify(plain.slice(0, 40)));
check("banner has blank lines above and below", /\n\n \u2588\u2588\u2588\u2588\u2588\u2588/.test(plain) && /\u2588\n\nask an LLM/.test(plain));
r = await worker.fetch(new Request("https://ask.dev/", { headers: { "user-agent": "curl/8.7.1" } }), ENV);
const colored = await r.text();
check("banner is red for a curl agent", colored.includes("\u001b[1m\u001b[31m"), JSON.stringify(colored.slice(0, 40)));
check("colour keeps the heredoc terminator clean", colored.includes("\n___\ncurl -s https://ask.dev/s | sh"));
r = await worker.fetch(new Request("https://ask.dev/?nc=1", { headers: { "user-agent": "curl/8.7.1" } }), ENV);
check("nc=1 disables colour", !(await r.text()).includes("\u001b["));
r = await worker.fetch(new Request("https://ask.dev/?color=1", { headers: { "user-agent": "Mozilla/5.0" } }), ENV);
check("color=1 forces colour on", (await r.text()).includes("\u001b["));
...
check("GET /models marks exactly one default", (body.match(/\(default\)/g) || []).length === 1, body);
r = await worker.fetch(new Request("https://ask.dev/models", { headers: { "user-agent": "Mozilla/5.0" } }), { ...ENV, ASK_MODEL: "smart" });
body = await r.text();
check("smart as default marks smart only, not think", /smart\s+\S+\s+\(default\)/.test(body) && !/think\s+\S+\s+\(default\)/.test(body), body);
r = await worker.fetch(new Request("https://ask.dev/models", { headers: { "user-agent": "Mozilla/5.0" } }), { ...ENV, ASK_MODEL: "openai/gpt-oss-120b" });
check("a raw model id as default marks no alias", !(await r.text()).includes("(default)"));
